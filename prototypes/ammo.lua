...
effect = lib.get_damage_trigger_effect(explosive_aoe.action.action_delivery.target_effects, "explosion")
if effect then
    effect.damage.amount = 4 -- AOE explosive damage
end

effect = lib.get_damage_trigger_effect(explosive_rocket.action.action_delivery.target_effects, "explosion")
if effect then
    effect.damage.amount = 5 -- Direct explosive damage
    table.insert(magmatic_rounds_magazine.ammo_type.action.action_delivery.target_effects, effect)
end

effect = lib.get_damage_trigger_effect(magmatic_rounds_magazine.ammo_type.action.action_delivery.target_effects, "physical")
if effect then
    effect.damage.amount = 20 -- Direct physical damage
end
