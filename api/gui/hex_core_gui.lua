...
function hex_core_gui.update_hex_core(player)
    ...
    if state.claimed then
        ...
        if locomotive_trading_unlocked then
            ...
            frame["hex-control-flow"]["allow-locomotive-trading"].tooltip = {"",
                {"hex-core-gui.allow-locomotive-trading-tooltip", "[font=heading-2][color=green]" .. storage.item_buffs.train_trading_capacity .. "[.color][.font]"},
                "\n\n",
                {"hex-core-gui.two-headed-trains-allowed", storage.train_trading.allow_two_headed_trains and 1 or 2},
            }
...
function hex_core_gui.on_quick_trade_button_click(player, elem)
    ...
    local total_removed, total_inserted, remaining_to_insert, total_coins_removed, total_coins_added = trades.process_trades_in_inventories(state.hex_core.surface.index, inv, inv, state.trades, check_output_buffer, nil, max_output_batches_per_trade, nil)
